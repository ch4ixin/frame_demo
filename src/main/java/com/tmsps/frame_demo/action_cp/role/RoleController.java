package com.tmsps.frame_demo.action_cp.role;
...
import com.tmsps.frame_demo.model.t_fk_role;
import com.tmsps.frame_demo.service.AuthService;
import com.tmsps.frame_demo.service.RoleService;
...
public class RoleController extends ProjBaseAction {
	...
	public void add(t_fk_role role) {
		...
		if (role_ids == null || role_ids.equals("")) {
			...
			for (int i = 0; i < kids.length; i++) {
				boolean flag = true;
				t_fk_role roleDb = bs.findById(kids[i], t_fk_role.class);
	...
	public void edit(t_fk_role role) {
		t_fk_role roleDb = bs.findById(role.getKid(), t_fk_role.class);
			...
			for (int j = 0; j < codes.length; j++) {
				...
				for (int k = 0; k < auth_codesDbs.length; k++) {
					if (auth_code.equals(auth_codesDbs[k])) {
						...
						if (auth != null) {
							if (auth.getUrl() != null && !("".equals(auth.getUrl().trim()))) {
