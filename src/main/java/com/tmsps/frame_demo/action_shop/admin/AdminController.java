package com.tmsps.frame_demo.action_shop.admin;
...
import com.tmsps.frame_demo.service.ShopAdminService;
...
public class AdminController extends ProjBaseAction {
	...
	public void pwd(String currentpwd, String confirm_password) {
		...
		if (MD5Util.MD5(currentpwd).equals(currentUser.getPwd())) {
			t_shop_admin userDb = bs.findById(currentUser.getKid(), t_shop_admin.class);
	...
	public void edit(t_shop_admin admin) {
		...
		adminDb.setUnits_id(admin.getUnits_id());
		adminDb.setRole_id(admin.getRole_id());
		adminDb.setCodes(admin.getCodes());
