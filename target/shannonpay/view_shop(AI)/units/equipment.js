...
//	var sm = new Ext.selection.CheckboxModel({});//		checkOnly : false,singleSelect : true
	...
	var equipmentDataStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
			type : "ajax",
			url : "/shannonpay/shop/ai/units/equipment/list_data.htm",
			reader : {
				type : 'json',
				root : 'list',
	...
	var equipmentStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
			type : "ajax",
			url : "/shannonpay/shop/ai/units/equipment/list_data_combox.htm",
			reader : {
				type : 'json',
				root : 'list',
	...
	function myEquipmentDel(kid) {
		Ext.Msg.confirm("提示:", "确定删除选定的记录?", function(e) {
			if (e == "yes") {
				Ext.Ajax.request({
					url : "/shannonpay/shop/ai/units/equipment/del.htm?kid=" + kid,
	...
	var add_equipment_form_panel = Ext.create("Ext.form.Panel", {
		url : "/shannonpay/shop/ai/units/equipment/add.htm",
		buttonAlign : "center",
...
//				console.log(equipment_ids);
	...
	var edit_equipment_form_panel = Ext.create("Ext.form.Panel", {
		url : "/shannonpay/shop/ai/units/equipment/edit.htm",
		buttonAlign : "center",
		bodyStyle : "padding: 10px;",
		width : 700,
		defaultType : "textfield",
...
//				console.log(equipment_ids);
	...
	function myEquipmentEdit(kid) {
		...
		Ext.Ajax.request({
			url : "/shannonpay/shop/ai/units/equipment/edit_form.htm?kid=" + kid,
			success : function(response) {
				var json = Ext.JSON.decode(response.responseText);
